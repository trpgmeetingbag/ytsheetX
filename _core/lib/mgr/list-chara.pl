...
## カバー検索
my $cover_query = decode('utf8', $::in{cover});
if($cover_query) { @list = grep { (split(/<>/))[10] =~ /\Q$cover_query\E/ } @list; }
$INDEX->param(cover => $cover_query);

## 機体名検索
my $mecha_query = decode('utf8', $::in{mechaName});
if($mecha_query) { @list = grep { (split(/<>/))[16] =~ /\Q$mecha_query\E/i } @list; }
$INDEX->param(mechaName => $mecha_query);
...
$id, undef, undef, $updatetime, $name, $player, $group, #0-6
    $image, $tags, $hide, #7-9
    $cover, $gender, $age, #10-12 （$raceを$coverに変更）
    $exp, $level, #13-14
    $classes, #15
    $mechaName, $areatag, #16-17 （$homeを$mechaに変更）
    $guild, $payment, #18-19
    $session,
  ) = (split /<>/, $_)[0..20];
  ...
  ## 通常リスト
    ...
    # ▼ ここからクラスの厳密な太字判定処理 ▼
    my @classes_raw = split(/[\/／]+/, $classes); # / で区切られていると想定
    my @classes_formatted;
    foreach my $c (@classes_raw) {
      $c =~ s/^\s+|\s+$//g; # 前後の空白除去
      # %data::class の情報を使って、ガーディアンクラスか厳密に判定
...
my @characters;
    push(@characters, {
      ...
      "PLAYER" => $player,
      "GROUP" => $group,
      "COVER" => $cover || '',   # $raceの代わりに$cover
      "MECHA" => $mechaName || '',   # 追加
      "GENDER" => $gender,
      "AGE" => $age,
      "EXP" => $exp,
      "LV" => $level,
      "CLASS_DISP" => $class_disp, # メイン・サポの代わりに統合されたHTMLを渡す
      "TAGS" => $tags_links,
      "DATE" => $updatetime,
      "HIDE" => $hide,
    });
...
$INDEX->param(ogDescript => 
  ...
  ($cover_query ? "カバー「${cover_query}」 " : '') .
  ($mecha_query ? "機体名「${mecha_query}」 " : '') .
