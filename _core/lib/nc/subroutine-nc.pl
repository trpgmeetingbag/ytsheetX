...
sub createUnitStatus {
  my %pc = %{$_[0]};
  # 記憶のカケラ（memory${num}Name）の入力済み個数をカウント
  my $memoryCount = 0;
  foreach my $i (1 .. $pc{memoryNum}) {
    if ($pc{"memory${i}Name"} ne '') {
      $memoryCount++;
    }
  }
  ...
  # リソースの出力設定
  my @unitStatus = (
    { '行動値' => $pc{initiativeTotal} . '/' . $pc{initiativeTotal} },
    { '最大行動値' => $pc{initiativeTotal} . '/' . $pc{initiativeTotal} },
    { '狂気点減少可能数(対話判定)' => $memoryCount . '/' . $memoryCount },
    { '記憶のカケラ' => $memoryCount . '/6' },
  );
