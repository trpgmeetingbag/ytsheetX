...
sub dataConvert {
  ...
  if($set_url =~ m"(^https?://charasheet\.vampire-blood\.net/m?[a-f0-9]+)"){
    ...
    # # ====== ▼ デバッグ用：JSONの内容を画面に出力して止める ▼ ======
    # if ($in{'game'} eq 'nechro') {
    #   my $debug_out = "<div style='text-align:left; font-family:monospace; font-size:14px; line-height:1.5;'>";
    #   $debug_out .= "<h3>保管所のJSONデータ一覧（ネクロニカ）</h3>";
      
    #   # キーをアルファベット順に並び替えてすべて出力
    #   foreach my $key (sort keys %in) {
    #     my $val = $in{$key};
    #     if (ref($val) eq 'ARRAY') {
    #       # 中身が配列（スキルや未練など複数あるもの）の場合はカンマ区切りにする
    #       $debug_out .= "<b style='color:#0055aa;'>$key</b> : [" . join(', ', @$val) . "]<br>";
    #     } else {
    #       # 通常の値
    #       $debug_out .= "<b style='color:#aa5500;'>$key</b> : $val<br>";
    #     }
    #   }
    #   $debug_out .= "</div>";
      
    #   # ゆとシートのシステム関数 error() を使って画面に強制出力し、処理を止める
    #   error($debug_out);
    # }
    # # ====== ▲ デバッグ用ここまで ▲ ======
    
    # game属性を見てネクロニカだったら専用コンバータへ
    if ($in{'game'} eq 'nechro') {
      return convertHokanjoToYtsheetNechronica(\%in);
    }

    return convertHokanjoToYtsheet(\%in);
  }
  ## キャラクターシート倉庫
  if($set_url =~ m"^https?://character-sheets\.appspot\.com/nc/edit.html"){
...
sub convertHokanjoToYtsheetNechronica {
  my $in = shift;
  my %pc;

  # --- 基本情報 ---
  $pc{characterName} = $in->{pc_name};
  $pc{age}           = $in->{age};
  $pc{height}        = $in->{pc_height};
  $pc{weight}        = $in->{pc_weight};
  $pc{tags}          = $in->{pc_tags};
  $pc{viewDamage} = 'gray';

  
  
  # 享年/暗示（保管所の pc_carma に「人形」などが入っている）
  $pc{anji}          = '【'.$in->{pc_carma}.'】' if $in->{pc_carma};
  ...
  # クラス（ポジション・メイン・サブ）
  $pc{position}  = $in->{Position_Name};
  $pc{mainClass} = $in->{MCLS_Name};
  $pc{subClass}  = $in->{SCLS_Name};

  # --- 作成ボーナス ---
  # 保管所の ST_Bonus (1:武装, 2:変異, 3:改造) をゆとシートの sttPre に変換
  if ($in->{ST_Bonus}) {
    my %stt_bonus_map = (
      '1' => 'buso',
      '2' => 'heni',
    ...
    $pc{sttPre} = $stt_bonus_map{$in->{ST_Bonus}};
  }
  ...
  # --- メモ・履歴系 ---
  $pc{freeNote}    = $in->{pc_making_memo};
  $pc{expTotal}    = $in->{exp_his_sum};  # 消費経験点

  # --- 記憶のカケラ ---
  # 保管所では kakera_name, kakera_memo に配列で入っている
  if (ref $in->{kakera_name} eq 'ARRAY') {
    my $mem_count = 1;
    for (my $i = 0; $i < scalar(@{$in->{kakera_name}}); $i++) {
      next if !$in->{kakera_name}->[$i]; # 空は飛ばす
      $pc{"memory${mem_count}Name"} = $in->{kakera_name}->[$i];
      $pc{"memory${mem_count}Note"} = $in->{kakera_memo}->[$i];
      $mem_count++;
    }
    $pc{memoryNum} = $mem_count - 1;
  }

  # カルマ carma_name, carma_memo, carma_completed
  if (ref $in->{carma_name} eq 'ARRAY') {
    my $karma_count = 1;
    for (my $i = 0; $i < scalar(@{$in->{carma_name}}); $i++) {
      next if !$in->{carma_name}->[$i];
      $pc{"karma${karma_count}Name"} = $in->{carma_name}->[$i];
      # $pc{"karma${karma_count}Check"} = $in->{carma_completed}->[$i] eq '1' ? 1 : 0; # 必要であれば
      $karma_count++;
    }
    $pc{karmaNum} = $karma_count - 1;
  }


  # --- 未練 ---
  # roice_name, roice_damage(狂気点), roice_pos(感情), roice_neg(発狂), roice_break(破壊時)
  if (ref $in->{roice_name} eq 'ARRAY') {
    my $miren_count = 1;
    for (my $i = 0; $i < scalar(@{$in->{roice_name}}); $i++) {
      next if !$in->{roice_name}->[$i];
      $pc{"miren${miren_count}Name"}  = $in->{roice_name}->[$i];
      $pc{"miren${miren_count}Insanity"}  = $in->{roice_damage}->[$i];
      if ($in->{roice_pos}->[$i]) {
        $pc{"miren${miren_count}Note"} = '【' . $in->{roice_pos}->[$i] . '】';
      }
      $pc{"miren${miren_count}Burst"}  = "''".$in->{roice_neg}->[$i]."：''".$in->{roice_break}->[$i];

      $miren_count++;
    }
    $pc{mirenNum} = $miren_count - 1;
  }

  # --- スキル・パーツ ---
  # 保管所のスキル系は Power_name, Power_timing, Power_cost 等の配列
  if (ref $in->{Power_name} eq 'ARRAY') {
    # 1. 各要素を一度隔離し、ひとまとめの配列（ハッシュの配列）にする
    my @powers = ();
    for (my $i = 0; $i < scalar(@{$in->{Power_name}}); $i++) {
      push @powers, {
        name    => $in->{Power_name}->[$i],
        hantei  => $in->{Power_hantei}->[$i] || 0,
        type    => $in->{Power_Type}->[$i],
        timing  => $in->{Power_timing}->[$i],
        cost    => $in->{Power_cost}->[$i],
        range   => $in->{Power_range}->[$i],
        memo    => $in->{Power_memo}->[$i],
        shozoku => $in->{Power_shozoku}->[$i]
      };
    }

    # 2. Power_hantei をキーとして昇順に並べ替え
    @powers = sort { $a->{hantei} <=> $b->{hantei} } @powers;

    # 3. 値の割り当て
    my $skill_count = 1;
    foreach my $pow (@powers) {
      my $h = $pow->{hantei};
      
      # 0と8以上（異常値）、および名前が空のデータは無視
      next if ($h <= 0 || $h >= 8);
      next if (!$pow->{name} || $pow->{name} eq '');

      my $position_val = '';
      my $category_val = '';
      
      # Power_hantei の値によって処理を変更
      if ($h == 1) {
        $position_val = 'skill';
        $category_val = $in->{Position_Name}; # ポジション名（ソロリティなど）
      } elsif ($h == 2) {
        $position_val = 'skill';
        $category_val = $in->{MCLS_Name};     # メインクラス名（ロマネスクなど）
      } elsif ($h == 3) {
        $position_val = 'skill';
        $category_val = $in->{SCLS_Name};     # サブクラス名（タナトスなど）
      } elsif ($h == 4) {
        $position_val = 'head';   # 頭パーツ
      } elsif ($h == 5) {
        $position_val = 'arms';   # 腕パーツ
      } elsif ($h == 6) {
        $position_val = 'torso';  # 胴パーツ
      } elsif ($h == 7) {
        $position_val = 'legs';   # 足パーツ
      }



      # NameとPositionを入力
      $pc{"skill${skill_count}Name"}     = $pow->{name};
      $pc{"skill${skill_count}Position"} = $position_val;
      $pc{"skill${skill_count}Source"} = $category_val if $category_val;
      $pc{"skill${skill_count}Cost"}     = $pow->{cost};
      $pc{"skill${skill_count}Range"}     = $pow->{range};
      $pc{"skill${skill_count}Note"}     = $pow->{memo};
      ...
      if ($h >= 4 && $h <= 7) {
        ...
        # ① 「基本」が含まれる場合
        if ($sz =~ /基本/) {
          $pc{"skill${skill_count}Source"} = '基本パーツ';
        }
        # ② 「武装・変異・改造」が含まれる場合
        elsif ($sz =~ /(武装|変異|改造)/) {
          $pc{"skill${skill_count}Source"} = $1; # マッチした文字（武装など）を設定
          
          # 数値の抽出（全角数字が混ざっていた場合を考慮して半角に変換してから抽出）
          my $sz_num = $sz;
          $sz_num =~ tr/０-９/0-9/;
          my @nums = ($sz_num =~ /(\d+)/g);
          
          # ③ 数値が1つだけ含まれていればLvに設定（④ 2つ以上なら何もしない）
          if (scalar(@nums) == 1) {
            $pc{"skill${skill_count}Lv"} = $nums[0];
          }
        }
        # ⑤ どちらにも引っかからない場合
        else {
          $pc{"skill${skill_count}Source"} = $sz;
        }
      }

      # ▼▼ タイミングの変換 ▼▼
      my $t_code = $pow->{timing};
      if (defined $t_code && $t_code ne '') {
        my %timing_map = (
          '0' => 'オート',
          '1' => 'アクション',
          '2' => 'ジャッジ',
        ...
        # 定義された数値であれば対応する文字列に変換、それ以外ならそのままセット
        if (exists $timing_map{$t_code}) {
          $pc{"skill${skill_count}Timing"} = $timing_map{$t_code};
        } else {
          $pc{"skill${skill_count}Timing"} = $t_code;
        }
      }

      # ▼▼ カテゴリ(Category) の判定 ▼▼
      my $type_code = $pow->{type};
      if (defined $type_code && $type_code ne '') {
        my %type_map = (
          '1' => '通常技',
          '2' => '必殺技',
          '3' => '行動値増加',
          '4' => '補助',
          '5' => '妨害',
          '6' => '防御／生贄', # CSSに合わせて全角スラッシュにしています
          '7' => '移動'
        );
        if (exists $type_map{$type_code}) {
          $pc{"skill${skill_count}Category"} = $type_map{$type_code};
        }
      }

      # ▼▼ 上昇行動値(Initiative) の判定 ▼▼
      my $p_name = $pow->{name};
      
      if ($p_name eq 'のうみそ') {
        $pc{"skill${skill_count}Initiative"} = 2;
      } 
      elsif ($p_name eq 'めだま') {
        $pc{"skill${skill_count}Initiative"} = 1;
      } 
      # Act_parts_A, B, C と名前が一致する場合はそれぞれのEf（効果値）を設定
      # ※空欄の場合に誤動作しないよう、名前が設定されているかも確認しています
      elsif ($in->{Act_parts_A} ne '' && $p_name eq $in->{Act_parts_A}) {
        $pc{"skill${skill_count}Initiative"} = $in->{Act_PartsA_Ef};
      } 
      elsif ($in->{Act_parts_B} ne '' && $p_name eq $in->{Act_parts_B}) {
        $pc{"skill${skill_count}Initiative"} = $in->{Act_PartsB_Ef};
      } 
      elsif ($in->{Act_parts_C} ne '' && $p_name eq $in->{Act_parts_C}) {
        $pc{"skill${skill_count}Initiative"} = $in->{Act_PartsC_Ef};
      }
    ...
    # 最終的なスキル・パーツの総数をセット
    $pc{skillNum} = $skill_count - 1;
  }

  # --- セッション履歴 ---
  if (ref $in->{get_exp_his} eq 'ARRAY') {
    my $hist_count = 1;
    
    for (my $i = 0; $i < scalar(@{$in->{get_exp_his}}); $i++) {
      my $exp  = $in->{get_exp_his}->[$i];
      my $note = $in->{seicho_memo_his}->[$i];
      
      # 経験点が空（または0）かつ、メモも空の完全な未入力行は読み飛ばす
      next if ((!defined $exp || $exp eq '' || $exp eq '0') && (!defined $note || $note eq ''));

      # ゆとシートの変数にセット
      $pc{"history${hist_count}Exp"}  = $exp;
      $pc{"history${hist_count}Note"} = $note;
      
      $hist_count++;
    }
    
    # 有効なセッション履歴の総数をセット
    $pc{historyNum} = $hist_count - 1;
  }
