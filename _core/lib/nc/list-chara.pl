...
# ▼ 修正後（+130を削除）
  if($exp_min_query =~ /^[+-]?[0-9]+$/) { @list = grep { (split(/<>/))[7] >= $exp_min_query } @list; }
  if($exp_max_query =~ /^[+-]?[0-9]+$/) { @list = grep { (split(/<>/))[7] <= $exp_max_query } @list; }
...
foreach (@list) {
  my (
      ...
      $exp, undef, $age, $height, $weight, $anji,             # 7-12 （実際のデータ通りに受け取る）
      $syndrome, undef,                                       # 13-14 （13にクラス情報が入ってくる）
  ...
  ## 通常リスト
    ...
    # クラス：ポジションとクラスを分割（$syndrome の中には「アリス / ステーシー / レクイエム」などが入っています）
    my @syn_arr = split('\s*/\s*', $syndrome);
    my $works      = $syn_arr[0] || ''; 
    my $main_class = $syn_arr[1] || '';
    my $sub_class  = $syn_arr[2] || '';
    
    # CSSで2段にするために span で囲む
    my $syndrome_html = "<span>${main_class}</span>";
    $syndrome_html .= "<span>${sub_class}</span>" if $sub_class;
...
"ID" => $id,
      ...
      "EXP" => $exp,
      "AGE" => $age,
      "ANJI" => $anji,                # ★正しく暗示が入ります
      "WORKS" => $works,              # ★正しくポジションが入ります
      "SYNDROME" => $syndrome_html,   # ★正しくクラスが入ります
      "TAGS" => $tags_links,
      "HIDE" => $hide,
      "DATE" => $updatetime,
    });
