...
## ●各種ファイルへのパス
  ...
  our $lib_palette_sub = $::core_dir . '/lib/nc/palette-sub.pl';
  # 一覧
  our $lib_list_char   = $::core_dir . '/lib/nc/list-chara.pl';
  # JSON出力
  our $lib_json     = $::core_dir . '/lib/json.pl';
  our $lib_json_sub = $::core_dir . '/lib/nc/json-sub.pl';
  # コンバート
  our $lib_convert = $::core_dir . '/lib/nc/convert.pl';

  # 各種データ
  our $data_class = $::core_dir . '/lib/nc/data-class.pl';  # シンドロームのデータ

  # HTMLテンプレート
  our $skin_tmpl  = $::core_dir . '/skin/nc/index.html';      # 一覧／登録フォーム等の大枠
  our $skin_sheet = $::core_dir . '/skin/nc/sheet-chara.html';   # キャラクターシート
