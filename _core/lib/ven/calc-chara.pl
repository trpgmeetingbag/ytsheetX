...
sub dataCalc {
  ...
  foreach my $num (1 .. $pc{weaponNum}){
    push(@customNoteKeys, map { "weapon${num}Custom${_}Note" } 1 .. $pc{"weapon${num}CustomNum"});
  }

  # 2.0の共通関数で一括変換
  convertNewlinesToBrTag(\%pc,
    qw/words freeNote freeHistory chatPalette bodyArrange/,
  ...
  # ▼ 一覧用のクレジット・維持費計算（view-charaと同じ安全な四則演算） ▼
  my $total_income = 0;
  my $total_expense = 0;
  my $history_debt = 0;
  ...
  # ▼ 2.0式 %NL を用いたエスケープと文字数制限 ▼
  my %NL;
  $NL{name}  = ($pc{aka} ? "“$pc{aka}”" : "").$pc{characterName};
  $NL{$_} = $pc{$_} foreach ('playerName','gender','age','attribute');
  foreach (keys %NL){
    $NL{$_} =~ s/[|｜]([^|｜]+?)《.+?》/$1/g;
  ...
  $NL{attribute}  = substr($NL{attribute}, 0, 20).'..' if length($NL{attribute}) > 20;

  # ▼ 2.0式一覧データの書き込み ($::updatedLine) ▼
  $::updatedLine =
    "$pc{id}<>$::file<>"
    . "$pc{birthTime}<>$::now<>$NL{name}<>$NL{playerName}<>$pc{group}<>"
    . "$pc{level}<>$NL{attribute}<>$NL{age}<>$NL{gender}<>$origin_list<>"
